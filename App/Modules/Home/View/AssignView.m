...
- (void)createUI{
    ...
    if (_model.recovered && _model.statusChangeAccountInfo) {
     ...
     self.nameLab.textColor = PWTextBlackColor;
        self.nameLab.text = [NSString stringWithFormat:NSLocalizedString(@"local.issue.issueFixed", @""),name];
...
-(void)repair{
   ...
   self.nameLab.text =[NSString stringWithFormat:NSLocalizedString(@"local.issue.issueFixed", @""),userManager.curUserInfo.name];
    [self recoveredUI];
}
/*
