...
@implementation PWImageGroupCell
-(instancetype)initWithItem:(PWImageGroupItem *)item{
    if(self = [super init]){
        ...
        if (_item.fromImageStr) {
            [_mImageView sd_setImageWithURL:[NSURL URLWithString:_item.fromImageStr] placeholderImage:[UIImage imageNamed:@""]];
        }else{
...
-(void)GestureRecognizerPressed:(UITapGestureRecognizer *)sender{
    if(_imageCelldelegate && [_imageCelldelegate respondsToSelector:@selector(PWImageGroupCellImageClick:gesture:)]){
        [_imageCelldelegate PWImageGroupCellImageClick:_item.itemTag gesture:sender];
    }
}
...
//图片展开视图
@implementation PWImageGroupView
...
-(instancetype)initWithGroupItems:(NSArray *)groupItems currentIndex:(NSInteger)currentIndex{
    if(self = [super init]){
        ...
        for(int i=0;i<_groupItems.count;++i){
            PWImageGroupItem *item = _groupItems[i];
            
            if(item.imageType == PWImageGroupImage){
                PWImageGroupCell *imageCell = [[PWImageGroupCell alloc]initWithItem:item];
...
- (void)layoutSubviews{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if(self.currentItem.imageType == PWImageGroupImage){
            PWImageGroupCell *imageCell = self.mScrollView.subviews[self.currentIndex];
...
-(void)PWImageGroupCellImageClick:(NSInteger)index gesture:(UITapGestureRecognizer *)gesture{
    ...
    PWImageGroupCell *imageCell = _mScrollView.subviews[index-10];
...
//-(void)PWVideoViewImageButtonClick:(UIButton *)sender item:(SSImageGroupItem *)item{
//
//    CGRect fromRect1 = [_mScrollView convertRect:_fromImgView.frame fromView:_fromImgView.superview];
//    CGRect fromRect2 = [_mScrollView convertRect:_fristImgView.frame fromView:_fristImgView.superview];
//
//    PWVideoView  *videoView = _mScrollView.subviews[item.itemTag-10];
//
//    [UIView animateWithDuration:0.3 animations:^{
//        self.backView.alpha = 0.01;
//        if(fromRect1.origin.y!=0){
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if(_currentPage!=_currentIndex){
        if(_currentItem.imageType == PWImageGroupImage){
            PWImageGroupCell *imageCell = _mScrollView.subviews[_currentIndex];
...
-(void)setScrollerViewLandscape{
    ...
    for(int i=0;i<_mScrollView.subviews.count;++i){
        PWImageGroupItem *item = _groupItems[i];
        if(item.imageType == PWImageGroupImage){
            PWImageGroupCell *imageCell = _mScrollView.subviews[i];
