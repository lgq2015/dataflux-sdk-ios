 ...
 */
- (instancetype)initDatePackerWithStartHour:(NSString *)startHour endHour:(NSString *)endHour period:(NSInteger)period selectedHour:(NSString *)selectedHour selectedMin:(NSString *)selectedMin tag:(NSInteger)tag{
    if (self = [super init]) {
...
- (void)initAppreaence {
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        [button setTitle:i == 0 ? NSLocalizedString(@"local.cancel", @"") : NSLocalizedString(@"local.confirm", @"") forState:UIControlStateNormal];
        if (i == 0) {
            [button setTitleColor:PWCancelBtnColor forState:UIControlStateNormal];
        } else {
            [button setTitleColor:PWBlueColor forState:UIControlStateNormal];
        }
    ...
    pickerView.backgroundColor =PWBackgroundColor;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i==0) {
            label.frame = CGRectMake(kWidth/2 - 40, 260/2-15, 30, 30);
        }else{
            label.frame = CGRectMake(kWidth/2 + 53, 260/2-15, 30, 30);
        }
...
- (void)buttonTapped:(UIButton *)sender {
    if (sender.tag == 10) {
        ...
        if ([self.delegate respondsToSelector:@selector(timePickerViewDidSelectRow:tag:)]) {
            [self.delegate timePickerViewDidSelectRow:restr tag:self.viewTag];
        }
        [self dismiss];
    }
}
...
-(UIView*)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    UIView *bg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kWidth/2, 30)];
    
    //添加一个label
    UILabel * label = [[UILabel alloc] init];
    
    if (component == 0) {
        label.frame = CGRectMake(kWidth/2-80, 0, 40, 30);
    }else{
