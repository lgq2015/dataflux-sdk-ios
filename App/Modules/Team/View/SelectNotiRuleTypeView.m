...
@interface SelectNotiRuleTypeView()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
@property (nonatomic, assign) CGFloat topCons;
@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) UITableView *mTableView;
@property (nonatomic, strong) NSArray *dataSource;
@property (nonatomic, assign) NotiRuleStyle style;
@end
@implementation SelectNotiRuleTypeView
...
-(UITableView *)mTableView{
    if (!_mTableView) {
        _mTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, kWidth, ZOOM_SCALE(162)) style:UITableViewStylePlain];
...
- (void)showInView:(UIView *)view notiRuleStyle:(NotiRuleStyle)style{
    ...
    self.dataSource =@[NSLocalizedString(@"local.BasicNotificationRule", @""),NSLocalizedString(@"local.DingNotificationRule", @""),NSLocalizedString(@"local.CustomNotificationRule", @"")];
    [self.mTableView reloadData];
    [_contentView setFrame:CGRectMake(0, -ZOOM_SCALE(162), kWidth,ZOOM_SCALE(162))];
    _contentView.alpha = 0;
    [UIView animateWithDuration:0.3 animations:^{
        self.hidden = NO;
        self.alpha = 1.0;
        _contentView.alpha =1.0;
        [_contentView setFrame:CGRectMake(0, 0, kWidth, ZOOM_SCALE(162))];
...
-(void)disMissView{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.contentView.frame = CGRectMake(0, -ZOOM_SCALE(162), kWidth, ZOOM_SCALE(162));
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BOOL  isSel = indexPath.row+1 == self.style?YES:NO;
    if (isSel) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.selectClick) {
        self.selectClick(indexPath.row+1);
    }
    [self disMissView];
