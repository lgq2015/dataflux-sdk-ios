//
//  RootViewController+ZYChangeNavColor.m
//  App
//
...
@implementation RootViewController (ZYChangeNavColor)
- (void)changeColor:(UIColor *)color scrolllView:(UIScrollView *)scrollView criticalValue:(CGFloat)value {
    ...
    if (offsetY > 2) {
        ...
        // 返回按钮颜色
        if (self.topNavBar.backBtn.isHidden == YES){
            self.topNavBar.backBtn.hidden = NO;
        }
        self.topNavBar.backBtn.alpha = alpha;
        // 分割线
        if (self.topNavBar.lineView.isHidden == YES){
            self.topNavBar.lineView.hidden = NO;
        }
...
- (void)zy_changeColor:(UIColor *)color scrolllView:(UIScrollView *)scrollView {
    // 默认临界值为30
