...
- (void)setArrow:(BOOL)arrow
{
    ...
    // 遍历子控件，改变其是否有箭头
    [self.subviews enumerateObjectsUsingBlock:^(PWCircle *circle, NSUInteger idx, BOOL *stop) {
...
-(void)lockViewPrepare{
    ...
    for (NSUInteger i=0; i<9; i++) {
        ...
        PWCircle *circle = [[PWCircle alloc] init];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self.circleSet enumerateObjectsUsingBlock:^(PWCircle *circle, NSUInteger idx, BOOL *stop) {
...
{
    return [(PWCircle *)[self.circleSet firstObject] state];
}

#pragma mark - 清空所有子控件的方向
- (void)resetAllCirclesDirect
{
    [self.subviews enumerateObjectsUsingBlock:^(PWCircle *obj, NSUInteger idx, BOOL *stop) {
        [obj setAngle:0];
    }];
}
...
{   PWCircle *circle =[self.circleSet lastObject];
    [circle setState:state];
    [self.circleSet replaceObjectAtIndex:self.circleSet.count-1 withObject:circle];
}
...
- (void)gestureEndByTypeSettingWithGesture:(NSString *)gesture length:(CGFloat)length
{
    if (length < CircleSetCountLeast) {
        ...
        NSString *gestureOne = [PWCircleViewConst getGestureWithKey:gestureOneSaveKey];
        
        if ([gestureOne length] < CircleSetCountLeast) { // 接收并存储第一个密码
            // 记录第一次密码
            [PWCircleViewConst saveGesture:gesture Key:gestureOneSaveKey];
            ...
            BOOL equal = [gesture isEqual:[PWCircleViewConst getGestureWithKey:gestureOneSaveKey]]; // 匹配两次手势
            ...
            if (equal){
                // 一致，存储密码
                [PWCircleViewConst saveGesture:gesture Key:gestureFinalSaveKey];
...
{
    NSString *password = [PWCircleViewConst getGestureWithKey:gestureFinalSaveKey];
...
- (NSString *)getGestureResultFromCircleSet:(NSMutableArray *)circleSet
{
    ...
    for (PWCircle *circle in circleSet) {
        // 遍历取tag拼字符串
...
- (void)connectCirclesInRect:(CGRect)rect lineColor:(UIColor *)color
{
    ...
    for (int index = 0; index < self.circleSet.count; index++) {
        ...
        // 取出选中按钮
        PWCircle *circle = self.circleSet[index];
...
-(void)calAngleAndconnectTheJumpedCircle{
    ...
    //取出最后一个对象
    PWCircle *lastOne = [self.circleSet lastObject];
    
    //倒数第二个
    PWCircle *lastTwo = [self.circleSet objectAtIndex:(self.circleSet.count -2)];
    ...
    PWCircle *centerCircle = [self enumCircleSetToFindWhichSubviewContainTheCenterPoint:center];
 ...
 *  @return 点所在的圆
 */
- (PWCircle *)enumCircleSetToFindWhichSubviewContainTheCenterPoint:(CGPoint)point
{
    PWCircle *centerCircle;
    for (PWCircle *circle in self.subviews) {
        if (CGRectContainsPoint(circle.frame, point)) {
            centerCircle = circle;
        }
    }
