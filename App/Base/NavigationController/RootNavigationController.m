...
+ (void)initialize
{
    ...
    [navBar setTitleTextAttributes:@{NSForegroundColorAttributeName :PWBlackColor, NSFontAttributeName : MediumFONT(18)}];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {
        if ([viewController conformsToProtocol:@protocol(PWTransitionProtocol)] && [self isNeedTransition:viewController]) {
...
-(id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    ...
    if ([fromVC conformsToProtocol:@protocol(PWTransitionProtocol)] && [toVC conformsToProtocol:@protocol(PWTransitionProtocol)]) {
        ...
        return transion;
    }else if([toVC conformsToProtocol:@protocol(PWTransitionProtocol)]){
...
-(BOOL)isNeedTransition:(UIViewController<PWTransitionProtocol> *)fromVC :(UIViewController<PWTransitionProtocol> *)toVC
{
    BOOL a = NO;
    BOOL b = NO;
...
//判断fromVC和toVC是否需要实现pinterest效果
-(BOOL)isNeedTransition:(UIViewController<PWTransitionProtocol> *)toVC
{
    BOOL b = NO;
    if ([toVC respondsToSelector:@selector(isNeedTransition)] && [toVC isNeedTransition]) {
        b = YES;
